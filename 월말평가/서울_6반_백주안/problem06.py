...
def sum_row_maximums(matrix):
    ...
    count = 0 # 최댓값들을 더한 값을 넣을 변수를 만듭니다. 
    for i in matrix: #matrix를 순회하며 리스트 목록을 조회하고, 순차정렬을 시킵니다. 
        i.sort()
        count += i[-1] #순차적으로 정렬되었다면 마지막 값이 가장 큰 값이기 때문에 마지막 값을 count에 더합니다.
    return count #count를 출력합니다.
